...
#include <functional>
#include <cmath>
#include "vector.hpp"  // Include the vector header
...
void print_help()
{
    ...
    cout << "Vector operations supported:" << endl;
    cout << "let v = vector{1, 2, 3};" << endl;
    cout << "v += vector{4, 5, 6};" << endl;
    cout << "v = v * scalar;" << endl;
    cout << "v = v + vector{7, 8, 9};" << endl << endl;
...
enum class TokenKind {
    ...
    comma, unary_math_func, binary_math_func, vector, vector_op, left_brace, right_brace
};
...
Token Token_stream::get()
{
    ...
    switch (ch) 
    {
        ...
        case '{': return Token(TokenKind::left_brace);
        case '}': return Token(TokenKind::right_brace);
        ...
        default:
            if (isalpha(ch)) 
            {
                ...
                if (s == "vector") return Token(TokenKind::vector);
                if (s == "+=" || s == "-=" || s == "*=" || s == "/=") return Token(TokenKind::vector_op, s);
...
{  
    double value;
    bool is_const;
    vector<double> vec_value;  // Allow for vector storage
    bool is_vector = false;

    Variable() : value(0), is_const(false), vec_value{} {}
    Variable(double v, bool c=false) : value(v), is_const(c), vec_value{}, is_vector(false) {}
    Variable(vector<double> v) : value(0), is_const(false), vec_value(v), is_vector(true) {}
};
...
double get_value(string s)
{
    ...
    return it->second.is_vector ? 0 : it->second.value;
}
...
vector<double> get_vector(string s)
{
    ...
    return it->second.is_vector ? it->second.vec_value : vector<double>{};
}
...
void set_vector(string s, vector<double> v)
{
    Variable& var = names.at(s);
    if (var.is_const) {
        error("set: cannot update constant vector ", s);
    }
    var.vec_value = v;
    var.is_vector = true;
}
...
void define_vector(string s, vector<double> v)
{
    names[s] = Variable(v);
}
        ...
        case TokenKind::vector:
            {
                vector<double> vec;
                Token next = ts.get();
                if (next.kind != TokenKind::left_brace)
                    error("'{' expected after vector name");
                ...
                if (next.kind != TokenKind::right_brace)
                    error("'}' expected after vector elements");
                define_vector(t.name, vec);  // Store the vector in the map
...
double expression()
{
    ...
    while(true) 
    {
        ...
        switch(t.kind) 
        {
            ...
            {
                vector<double> vec1 = get_vector(t.name);
                vector<double> vec2 = get_vector(t.name);
                // Handle the vector operation
                break;
            }
            default:
                ts.unget(t);
